...
class BoundingBoxTests: XCTestCase {
    ...
    func testEmpty() {
        ...
        XCTAssertTrue(BoundingBox(min: Point(x: 5, y: 3), max: Point(x: 4, y: 4)).isEmpty)
    }
    ...
    func testUnionBox() {
        var box1 = BoundingBox(p1: Point(x: 2.0, y: 3.0), p2: Point(x: 3.0, y: 5.0))
        let box2 = BoundingBox(p1: Point(x: 2.5, y: 6.0), p2: Point(x: 3.0, y: 8.0))
    ...
    func testUnionPoint() {
        let box1 = BoundingBox(p1: Point(x: -1, y: -1), p2: Point(x: 3, y: 5))
        var box2 = box1
        XCTAssertEqual(box2.union(Point(x: 0, y: -1)), box1)
        XCTAssertEqual(box2.union(Point(x: -2, y: 0)), BoundingBox(p1: Point(x: -2, y: -1), p2: Point(x: 3, y: 5)))
    ...
    func testIntersection() {
        let box1 = BoundingBox(p1: Point(x: 0, y: 0), p2: Point(x: 3, y: 2))
        let box2 = BoundingBox(p1: Point(x: 2, y: 1), p2: Point(x: 4, y: 5)) // overlaps box1
        let box3 = BoundingBox(p1: Point(x: 2, y: 4), p2: Point(x: 4, y: 5)) // does not overlap box1
        let box4 = BoundingBox(p1: Point(x: 3, y: 0), p2: Point(x: 5, y: 2)) // overlaps box1 exactly on x edge
        let box5 = BoundingBox(p1: Point(x: 0, y: 2), p2: Point(x: 3, y: 4)) // overlaps box1 exactly on y edge
        let box6 = BoundingBox(p1: Point(x: 0, y: 0), p2: Point(x: -5, y: -5)) // overlaps box1 only at (0,0)
        let expectedBox = BoundingBox(p1: Point(x: 2, y: 1), p2: Point(x: 3, y: 2))
        ...
        XCTAssertEqual(box1.intersection(box4), BoundingBox(p1: Point(x: 3, y: 0), p2: Point(x: 3, y: 2)))
        XCTAssertEqual(box1.intersection(box5), BoundingBox(p1: Point(x: 0, y: 2), p2: Point(x: 3, y: 2)))
        XCTAssertEqual(box1.intersection(box6), BoundingBox(p1: Point.zero, p2: Point.zero))
    }
}
