...
class PathTests: XCTestCase {
    ...
    override func tearDown() {
        ...
        func testInitCGPathRect() {
            ...
            let rect = Rect(origin: Point(x: 0, y: 0), size: CGSize(width: 1, height: 2))
            let cgPath1 = CGPath(rect: rect.cgRect, transform: nil)
        ...
        func testInitCGPathEllipse() {
            ...
            let rect = Rect(origin: Point(x: 0, y: 0), size: CGSize(width: 1, height: 2))
            let cgPath2 = CGPath(ellipseIn: rect.cgRect, transform: nil)
            let path2 = Path(cgPath: cgPath2)

            let p1 = Point(x: 1.0, y: 1.0)
            let p2 = Point(x: 0.5, y: 2.0)
            let p3 = Point(x: 0.0, y: 1.0)
            let p4 = Point(x: 0.5, y: 0.0)
        ...
        func testInitCGPathQuads() {
            ...
            let p3 = Point(x: 3.0, y: 0.5)
            let p4 = Point(x: 2.0, y: 0.0)
            let p5 = Point(x: 0.0, y: 0.0)
            let p6 = Point(x: -1.0, y: 0.5)
        ...
        func testGeometricProperties() {
            ...
            let path: Path = {
                ...
                mutablePath.addQuadCurve(to: Point(x: 4, y: 2),
                                         control: Point(x: 4, y: 1))
                mutablePath.addCurve(to: Point(x: 2, y: 1),
            ...
            let expectedLinePosition = Point(x: 2, y: 1)
            let expectedLineDerivative = Point(x: 1, y: 0)
            let expectedLineNormal = Point(x: 0, y: 1)
            ...
            let expectedQuadraticPosition = Point(x: 3.4375, y: 1.0625)
            let expectedQuadraticDerivative = Point(x: 1.5, y: 0.5)
            let expectedQuadraticNormal = Point(x: -0.316, y: 0.949)
            ...
            let expectedCubicPosition = Point(x: 2, y: 1)
            let expectedCubicDerivative = Point(x: 0, y: -6)
            let expectedCubicNormal = Point(x: 1, y: 0)
            ...
            let expectedPointPosition = Point(x: 1, y: 1)
            let expectedPointDerivative = Point.zero
        ...
        func testIntersections() {
            let circleCGPath = CGPath(ellipseIn: Rect(x: 2.0, y: 3.0, width: 2.0, height: 2.0).cgRect, transform: nil)
            let circlePath = Path(cgPath: circleCGPath) // a circle centered at (3, 4) with radius 2

            let rectangleCGPath = CGPath(rect: Rect(x: 3.0, y: 4.0, width: 2.0, height: 2.0).cgRect, transform: nil)
            ...
            XCTAssert(intersections.contains(Point(x: 4.0, y: 4.0)))
            XCTAssert(intersections.contains(Point(x: 3.0, y: 5.0)))
        }
        ...
        func testSelfIntersectsMultiComponentPath() {
            let multiComponentPath = { () -> Path in
                let cgPath = CGMutablePath()
                cgPath.addRect(Rect(x: 0, y: 0, width: 2, height: 4))
                cgPath.addRect(Rect(x: 1, y: 2, width: 2, height: 1))
                return Path(cgPath: cgPath)
            }()
        ...
        func testIntersectsOpenPathEdgeCase() {
            let openPath1 = Path(components: [PathComponent(curves: [LineSegment(p0: Point(x: 1, y: 3), p1: Point(x: 2, y: 5))])])
            let openPath2 = Path(components: [PathComponent(curves: [LineSegment(p0: Point(x: 2, y: 5), p1: Point(x: 9, y: 7))])])
            ...
            let closedPath1 = Path(cgPath: CGPath(rect: Rect(x: 2, y: 5, width: 1, height: 1).cgRect, transform: nil))
        ...
        func testSelfIntersectsOpenPathEdgeCase() {
            let cgPath = CGMutablePath()
            cgPath.addLines(between: [Point(x: 0, y: 0), Point(x: 1, y: 0), Point(x: 0, y: 1), Point(x: 0, y: -1)])
        ...
        func testEquatable() {
            ...
            let path1 = Path(cgPath: CGPath(rect: rect.cgRect, transform: nil))
        ...
        func testHashing() {
            ...
            let rect = Rect(origin: Point(x: -1, y: -1), size: CGSize(width: 2, height: 2))
            let path1 = Path(cgPath: CGPath(rect: rect.cgRect, transform: nil))
            let path2 = Path(cgPath: CGPath(rect: rect.cgRect, transform: nil))

            XCTAssertEqual(path1.hash, path2.hash)

            // path that is equal should be located in a set
            let path3 = path1.copy(using: AffineTransform.identity)
            let set = Set([path1])
        ...
        func testWindingCountBasic() {
            let rect1 = Path(cgPath: CGPath(rect: Rect(origin: Point(x: -1, y: -1), size: CGSize(width: 2, height: 2)).cgRect, transform: nil))
            let rect2 = Path(cgPath: CGPath(rect: Rect(origin: Point(x: -2, y: -2), size: CGSize(width: 4, height: 4)).cgRect, transform: nil))
            let path = Path(components: rect1.components + rect2.components)
            // outside of both rects
            XCTAssertEqual(path.windingCount(Point(x: -3, y: 0)), 0)
            // inside rect1 but outside rect2
            XCTAssertEqual(path.windingCount(Point(x: -1.5, y: 0)), 1)
            // inside both rects
            XCTAssertEqual(path.windingCount(Point(x: 0, y: 0)), 2)
        }
        ...
        func testWindingCountCornersNoAdjust() {
            ...
            let path1 = Path(cgPath: {
                ...
                temp.addLines(between: [Point(x: 0, y: 0),
                                        Point(x: 2, y: 0),
                                        Point(x: 2, y: 2),
                                        Point(x: 1, y: 1),
                                        Point(x: 0, y: 2)])
            ...
            XCTAssertEqual(path1.windingCount(Point(x: 1.5, y: 1)), 1)
            XCTAssertEqual(path1.reversed().windingCount(Point(x: 1.5, y: 1)), -1)
            ...
            XCTAssertEqual(path2.windingCount(Point(x: 1, y: 2)), 0)
            XCTAssertEqual(path2.reversed().windingCount(Point(x: 1, y: 2)), 0)
            // getting trickier ...
            ...
            XCTAssertEqual(path3.windingCount(Point(x: 3, y: 1)), 1)
            XCTAssertEqual(path3.reversed().windingCount(Point(x: 3, y: 1)), -1)
            let path4 = Path(cgPath: {
                let temp = CGMutablePath()
                temp.addLines(between: [Point(x: 2, y: 0),
                                        Point(x: 4, y: 0),
                                        Point(x: 4, y: 4),
                                        Point(x: 2, y: 2),
                                        Point(x: 0, y: 3)])
            ...
            XCTAssertEqual(path4.windingCount(Point(x: 2, y: 3)), 0)
            XCTAssertEqual(path4.reversed().windingCount(Point(x: 2, y: 3)), 0)
        }
        ...
        func testWindingCountCornersYesAdjust() {
            ...
            XCTAssertEqual(path1.windingCount(Point(x: 1, y: 2)), 1)
            XCTAssertEqual(path1.windingCount(Point(x: 3, y: 2)), 0)
            XCTAssertEqual(path1.reversed().windingCount(Point(x: 3, y: 2)), 0)
        }

        func testWindingCountExactlyParallel() {
            let path1 = Path(cgPath: {
                let temp = CGMutablePath()
                temp.addLines(between: [Point(x: 1, y: 0),
                                        Point(x: 2, y: 0),
                                        Point(x: 2, y: 2),
                                        Point(x: 0, y: 2),
                                        Point(x: 0, y: 1),
            ...
            XCTAssertEqual(path1.windingCount(Point(x: 0.5, y: 0)), 0)
            XCTAssertEqual(path1.windingCount(Point(x: 3, y: 0)), 0)
            XCTAssertEqual(path1.windingCount(Point(x: 1.5, y: 1)), 1)
            XCTAssertEqual(path1.windingCount(Point(x: 3, y: 1)), 0)
            XCTAssertEqual(path1.windingCount(Point(x: 3, y: 2)), 0)
            let path2 = path1.copy(using: AffineTransform(scaleX: 1, y: -1)).reversed()
            XCTAssertEqual(path2.windingCount(Point(x: 0.5, y: 0)), 0)
            XCTAssertEqual(path2.windingCount(Point(x: 3, y: 0)), 0)
            XCTAssertEqual(path2.windingCount(Point(x: 1.5, y: -1)), 1)
            XCTAssertEqual(path2.windingCount(Point(x: 3, y: -1)), 0)
            XCTAssertEqual(path2.windingCount(Point(x: 3, y: -2)), 0)
        }
        ...
        func testWindingCountCusp() {
            let path1 = Path(cgPath: {
                ...
                temp.addCurve(to: Point(x: 1, y: -1),
            ...
            // at the bottom
            XCTAssertEqual(path1.windingCount(Point(x: 0.5, y: -1)), 0)
            XCTAssertEqual(path1.windingCount(Point(x: 1.5, y: -1)), 0)
            XCTAssertEqual(path1.windingCount(Point(x: 4, y: -1)), 0)
            // between the y-coordinates of start and end
            XCTAssertEqual(path1.windingCount(Point(x: 0.49, y: -0.5)), 0)
            XCTAssertEqual(path1.windingCount(Point(x: 0.57, y: -0.5)), -1)
            XCTAssertEqual(path1.windingCount(Point(x: 1, y: -0.5)), 0)
            XCTAssertEqual(path1.windingCount(Point(x: 5, y: -0.5)), 0)
            // near the starting point
            XCTAssertEqual(path1.windingCount(Point(x: 0.01, y: -0.02)), 0)
            XCTAssertEqual(path1.windingCount(Point(x: 0.01, y: 0)), -1)
            XCTAssertEqual(path1.windingCount(Point(x: 0.01, y: 0.02)), 0)
            // around the self-intersection (0.280, 0.296) t = 0.053589836, 0.74641013
            XCTAssertEqual(path1.windingCount(Point(x: 0.279, y: 0.295)), 0)
            XCTAssertEqual(path1.windingCount(Point(x: 0.280, y: 0.295)), -1)
            XCTAssertEqual(path1.windingCount(Point(x: 0.281, y: 0.295)), 0)
            XCTAssertEqual(path1.windingCount(Point(x: 0.279, y: 0.296)), 0)
            XCTAssertEqual(path1.windingCount(Point(x: 0.280, y: 0.2961)), 1)
            XCTAssertEqual(path1.windingCount(Point(x: 0.280, y: 0.2959)), -1)
            XCTAssertEqual(path1.windingCount(Point(x: 0.281, y: 0.296)), 0)
            XCTAssertEqual(path1.windingCount(Point(x: 0.279, y: 0.297)), 0)
            XCTAssertEqual(path1.windingCount(Point(x: 0.280, y: 0.297)), 1)
            XCTAssertEqual(path1.windingCount(Point(x: 0.281, y: 0.297)), 0)
            // intersecting the middle of the loop
            XCTAssertEqual(path1.windingCount(Point(x: 0.9, y: 0.856)), 0)
            XCTAssertEqual(path1.windingCount(Point(x: 0.6, y: 0.856)), 1)
            XCTAssertEqual(path1.windingCount(Point(x: 0.1, y: 0.856)), 0)
            // around the y extrema (x : 0.6606065, y : 1.09017)
            let yExtrema = Point(x: 0.6606065, y: 1.09017)
            let smallValue = 1.0e-5
            XCTAssertEqual(path1.windingCount(yExtrema - Point(x: 0, y: smallValue)), 1)
            XCTAssertEqual(path1.windingCount(yExtrema - Point(x: smallValue, y: 0)), 0)
            XCTAssertEqual(path1.windingCount(yExtrema), 0)
            XCTAssertEqual(path1.windingCount(yExtrema + Point(x: smallValue, y: 0)), 0)
            XCTAssertEqual(path1.windingCount(yExtrema + Point(x: 4, y: 0)), 0)
            XCTAssertEqual(path1.windingCount(yExtrema + Point(x: 0, y: smallValue)), 0)
        }
        ...
        func testWindingCountQuadratic() {
            ...
            XCTAssertEqual(path.windingCount(Point(x: 0.1, y: 0)), 0)
            XCTAssertEqual(path.windingCount(Point(x: 3, y: 0)), 0)
            XCTAssertEqual(path.windingCount(Point(x: 0.99, y: 0.5)), 1)
            XCTAssertEqual(path.windingCount(Point(x: 1.01, y: 0.5)), 0)
            // near the starting point
            XCTAssertEqual(path.windingCount(Point(x: 1.99, y: 1)), 1)
            XCTAssertEqual(path.windingCount(Point(x: 2.01, y: 1)), 0)
            // near the X extrema
            XCTAssertEqual(path.windingCount(Point(x: 2.26, y: 2.0625)), 0)
            XCTAssertEqual(path.windingCount(Point(x: 2.24, y: 2.0625)), 1)
            // near the Y extrema
            XCTAssertEqual(path.windingCount(Point(x: 2.122449, y: 2.285713)), 1)
            XCTAssertEqual(path.windingCount(Point(x: 2.121, y: 2.285714)), 0)
            XCTAssertEqual(path.windingCount(Point(x: 2.123, y: 2.285714)), 0)
            XCTAssertEqual(path.windingCount(Point(x: 2.122449, y: 2.285715)), 0)
        }
        ...
        func testWindingCountCornerCase() {
            ...
            let path = Path(cgPath: {
                ...
                temp.addCurve(to: Point(x: 259.9693035427533, y: 32.74107137166386),
            ...
            let y = Double(24.268753616441533).nextUp // next higher floating point from bottom of path
            XCTAssertEqual(path.windingCount(Point(x: 268.5, y: y)), 0)
            XCTAssertEqual(path.windingCount(Point(x: 268.3, y: y)), 1)
        }
        ...
        func testWindingCountRealWorldIssue() {
            ...
            XCTAssertEqual(path.windingCount(Point(x: 595.8, y: y)), 0)
            XCTAssertEqual(path.windingCount(Point(x: 596.1, y: y)), 1)
            XCTAssertEqual(path.windingCount(Point(x: 597, y: y)), 2)
            XCTAssertEqual(path.windingCount(Point(x: 603.9411804326238, y: y)), 1) // the point that was failing (reported 2 instead of 1)
            XCTAssertEqual(path.windingCount(Point(x: 606, y: y)), 1)
            XCTAssertEqual(path.windingCount(Point(x: 607, y: y)), 0)
        }

        func testContainsSimple1() {
            let rect = Rect(origin: Point(x: -1, y: -1), size: CGSize(width: 2, height: 2))
            let path = Path(cgPath: CGPath(rect: rect.cgRect, transform: nil))
            XCTAssertFalse(path.contains(Point(x: -2, y: 0))) // the first point is outside the rectangle on the left
            XCTAssertTrue(path.contains(Point(x: 0, y: 0))) // the second point falls inside the rectangle
            XCTAssertFalse(path.contains(Point(x: 3, y: 0))) // the third point falls outside the rectangle on the right
            XCTAssertTrue(path.contains(Point(x: -0.99999, y: 0))) // just *barely* in the rectangle
        }

        func testContainsSimple2() {
            let rect = Rect(origin: Point(x: -1, y: -1), size: CGSize(width: 2, height: 2))
            let path = Path(cgPath: CGPath(ellipseIn: rect.cgRect, transform: nil))
            XCTAssertFalse(path.contains(Point(x: 5, y: 5))) // the first point is way outside the circle
            XCTAssertFalse(path.contains(Point(x: -0.8, y: -0.8))) // the second point is outside the circle, but within the bounding rect
            XCTAssertTrue(path.contains(Point(x: 0.3, y: 0.3))) // the third point falls inside the circle
            ...
            XCTAssertTrue(path.contains(Point(x: 0.5, y: 0.0), using: .evenOdd))
            XCTAssertTrue(path.contains(Point(x: 0.5, y: 0.0), using: .winding))

            // the 5th point falls outside the circle, but drawing a horizontal line has a glancing blow with it
            XCTAssertFalse(path.contains(Point(x: 0.1, y: 1.0), using: .evenOdd))
            XCTAssertFalse(path.contains(Point(x: 0.1, y: -1.0), using: .winding))
        }

        func testContainsStar() {
            let starPoints = stride(from: 0.0, to: 2.0 * Double.pi, by: 0.4 * Double.pi).map { Point(x: cos($0), y: sin($0)) }
            let cgPath = CGMutablePath()
            ...
            let outsidePoint = Point(x: 0.5, y: -0.5)
            ...
            XCTAssertTrue(path.contains(Point.zero, using: .winding))
            XCTAssertFalse(path.contains(Point.zero, using: .evenOdd))

            // check a point inside one of the star's arms
            let armPoint = Point(x: 0.9, y: 0.0)
            ...
            for i in 0 ..< 5 {
                let point = starPoints[i] + Point(x: 0.1, y: 0.0)
        ...
        func testContainsCircleWithHole() {
            let rect1 = Rect(origin: Point(x: -3, y: -3), size: CGSize(width: 6, height: 6))
            let circlePath = Path(cgPath: CGPath(ellipseIn: rect1.cgRect, transform: nil))
            let rect2 = Rect(origin: Point(x: -1, y: -1), size: CGSize(width: 2, height: 2))
            let reversedCirclePath = Path(cgPath: CGPath(ellipseIn: rect2.cgRect, transform: nil)).reversed()
            let circleWithHole = Path(components: circlePath.components + reversedCirclePath.components)
            XCTAssertFalse(circleWithHole.contains(Point(x: 0.0, y: 0.0), using: .evenOdd))
            XCTAssertFalse(circleWithHole.contains(Point(x: 0.0, y: 0.0), using: .winding))
            XCTAssertTrue(circleWithHole.contains(Point(x: 2.0, y: 0.0), using: .evenOdd))
            XCTAssertTrue(circleWithHole.contains(Point(x: 2.0, y: 0.0), using: .winding))
            XCTAssertFalse(circleWithHole.contains(Point(x: 4.0, y: 0.0), using: .evenOdd))
            XCTAssertFalse(circleWithHole.contains(Point(x: 4.0, y: 0.0), using: .winding))
        }
        ...
        func testContainsCornerCase() {
            ...
            let squareCenter = Point(x: 0.5, y: 0.5)
            let point1 = Point(x: -0.75, y: 1)
            let point2 = Point(x: 1.75, y: 2)
        ...
        func testContainsRealWorldEdgeCase() {
            ...
            let point = Point(x: 281.2936999253952, y: 221.7262912473492)
            let cgPath = CGMutablePath()
            cgPath.move(to: Point(x: 210.32116840649363, y: 106.4029658046467))
            cgPath.addLine(to: Point(x: 195.80672765188274, y: 106.4029658046467))
            cgPath.addLine(to: Point(x: 195.80672765188274, y: 221.7262912473492))
            cgPath.addLine(to: Point(x: 273.5510327577471, y: 221.72629124734914)) // !!! precision issues comes from fact line is almost, but not perfectly horizontal
            cgPath.addCurve(to: Point(x: 271.9933072984535, y: 214.38053683325302),
                            control1: Point(x: 273.05768924540223, y: 219.26088569867528),
                            control2: Point(x: 272.5391291486813, y: 216.81119916319818))
            cgPath.addCurve(to: Point(x: 252.80681257385964, y: 162.18313232371986),
                            control1: Point(x: 267.39734333475377, y: 195.3589483577662),
                            control2: Point(x: 260.947626989152, y: 177.936810624913))
            cgPath.addCurve(to: Point(x: 215.4444979991486, y: 111.76311400605556),
                            control1: Point(x: 242.1552743057946, y: 142.6678463672315),
                            control2: Point(x: 229.03183407884012, y: 126.09450622380493))
            cgPath.addCurve(to: Point(x: 210.32116840649363, y: 106.4029658046467),
        ...
        func testContainsRealWorldEdgeCase2() {
            ...
            let circlePath = Path(cgPath: { () -> CGPath in
                ...
                path.addCurve(to: Point(x: 368.8204391162164, y: 479.3753548709112),
                              control1: Point(x: 387.87721334546706, y: 471.5724761398741),
                              control2: Point(x: 379.1950200835358, y: 479.7108020561644))

                path.addCurve(to: Point(x: 350.64297941785986, y: 459.98313345640736),
                              control1: Point(x: 358.445858148897, y: 479.039907685658),
                              control2: Point(x: 350.30753223260666, y: 470.35771442372675))
                path.addCurve(to: Point(x: 370.0352008323637, y: 441.80567375805083),
                              control1: Point(x: 350.97842660311306, y: 449.60855248908797),
                              control2: Point(x: 359.66061986504434, y: 441.4702265727976))
                path.addCurve(to: Point(x: 388.21266053072026, y: 461.1978951725547),
                              control1: Point(x: 380.4097817996831, y: 442.14112094330403),
                              control2: Point(x: 388.54810771597346, y: 450.8233142052353))
                return path
            }())

            XCTAssertTrue(circlePath.contains(Point(x: 369, y: 459), using: .evenOdd))
            XCTAssertTrue(circlePath.contains(Point(x: 369, y: 459.9832416054124), using: .evenOdd)) // this is one that would fail in practice
            XCTAssertTrue(circlePath.contains(Point(x: 369, y: 458.9832416054124), using: .evenOdd))
        }

        func testContainsRealWorldEdgeCase3() {
            let point = Point(x: 207, y: 60.09055464612847) // point has to be chosen carefully to fall inside path bounding box or else it's excluded trivially
            let cgPath = CGMutablePath()
            cgPath.move(to: Point(x: 156.96601717963904, y: 61.6108671143393))
            cgPath.addCurve(to: Point(x: 158.48632964784989, y: 60.090554646128446),
                            control1: Point(x: 156.96601717963904, y: 60.77122172316883),
                            control2: Point(x: 157.6466842566794, y: 60.090554646128446))
            cgPath.addLine(to: Point(x: 206.74971723237456, y: 60.09055464612845))
            cgPath.addCurve(to: Point(x: 207.35854749702355, y: 63.13117958255016),
        ...
        func testContainsEdgeCaseParallelDerivative() {
            ...
            let cgPath = CGMutablePath()
            cgPath.move(to: Point(x: 0.0, y: 1.0))
            cgPath.addQuadCurve(to: Point(x: 1.0, y: 0.0), control: Point(x: 0, y: 0)) // quad curve has derivative exactly horizontal at t=1
            cgPath.addLine(to: Point(x: 2.0, y: -1.0e-5))
            cgPath.addLine(to: Point(x: 4.0, y: 1))
            cgPath.closeSubpath()
            let path = Path(cgPath: cgPath)
            XCTAssertTrue(path.contains(Point(x: 0.5, y: 0.5)))
            XCTAssertFalse(path.contains(Point(x: 3.0, y: 0.0)))
        }

        func testContainsPath() {
            let rect1 = Path(cgPath: CGPath(rect: Rect(x: 1, y: 1, width: 5, height: 5).cgRect, transform: nil))
            let rect2 = Path(cgPath: CGPath(rect: Rect(x: 2, y: 2, width: 3, height: 3).cgRect, transform: nil)) // fully contained inside rect1
            let rect3 = Path(cgPath: CGPath(rect: Rect(x: 2, y: 2, width: 5, height: 3).cgRect, transform: nil)) // starts inside, but not contained in rect1
            let rect4 = Path(cgPath: CGPath(rect: Rect(x: 7, y: 1, width: 5, height: 5).cgRect, transform: nil)) // fully outside rect1
        ...
        let p1 = Point(x: 1.0, y: 0.0) // intersection 1
        let p2 = Point(x: 2.0, y: 0.0)
        let p3 = Point(x: 2.0, y: 1.0) // intersection 2
        let p4 = Point(x: 2.0, y: 2.0)
        let p5 = Point(x: 0.0, y: 2.0)

        // points on the second square
        let p6 = Point(x: 1.0, y: -1.0)
        let p7 = Point(x: 3.0, y: -1.0)
        let p8 = Point(x: 3.0, y: 1.0)
        let p9 = Point(x: 1.0, y: 1.0)
        ...
        func testSubtractingWinding() {
            ...
            let path = Path(cgPath: {
                let cgPath = CGMutablePath()
                cgPath.addRect(Rect(x: 0, y: 0, width: 5, height: 5))
                cgPath.addRect(Rect(x: 1, y: 1, width: 3, height: 3))
                return cgPath
            }())
            let subtractionPath = Path(cgPath: CGPath(rect: Rect(x: 2, y: 2, width: 1, height: 1).cgRect, transform: nil))
        ...
        func testUnionCoincidentEdges1() {
            ...
            let square1 = Path(cgPath: CGPath(rect: Rect(x: 0, y: 0, width: 1, height: 1).cgRect, transform: nil))
            let square2 = Path(cgPath: CGPath(rect: Rect(x: 1, y: 0, width: 1, height: 1).cgRect, transform: nil))
        ...
        func testUnionCoincidentEdges2() {
            ...
            let square1 = Path(cgPath: CGPath(rect: Rect(x: 0, y: 0, width: 3, height: 3).cgRect, transform: nil))
            let square2 = Path(cgPath: CGPath(rect: Rect(x: 2, y: 1, width: 1, height: 1).cgRect, transform: nil))
            let expectedUnion = { () -> Path in
                let temp = CGMutablePath()
                temp.move(to: Point.zero)
                temp.addLine(to: Point(x: 3.0, y: 0.0))
                temp.addLine(to: Point(x: 3.0, y: 1.0))
                temp.addLine(to: Point(x: 3.0, y: 2.0))
                temp.addLine(to: Point(x: 3.0, y: 3.0))
                temp.addLine(to: Point(x: 0.0, y: 3.0))
                temp.closeSubpath()
                return Path(cgPath: temp)
            }()
        ...
        func testUnionCoincidentEdges3() {
            ...
            let square1 = Path(cgPath: CGPath(rect: Rect(x: 0, y: 0, width: 3, height: 3).cgRect, transform: nil))
            let square2 = Path(cgPath: CGPath(rect: Rect(x: 3, y: 2, width: -2, height: 2).cgRect, transform: nil))
            let expectedUnion = { () -> Path in
                let temp = CGMutablePath()
                temp.move(to: Point.zero)
                temp.addLine(to: Point(x: 3.0, y: 0.0))
                temp.addLine(to: Point(x: 3.0, y: 2.0))
                temp.addLine(to: Point(x: 3.0, y: 3.0))
                temp.addLine(to: Point(x: 3.0, y: 4.0))
                temp.addLine(to: Point(x: 1.0, y: 4.0))
                temp.addLine(to: Point(x: 1.0, y: 3.0))
                temp.addLine(to: Point(x: 0.0, y: 3.0))
                temp.closeSubpath()
                return Path(cgPath: temp)
            }()
        ...
        func testUnionCoincidentEdgesRealWorldTestCase1() {
            let polygon1 = { () -> Path in
                let temp = CGMutablePath()
                temp.addLines(between: [Point(x: 111.2, y: 90.0),
            ...
            let polygon2 = { () -> Path in
                let temp = CGMutablePath()
                temp.addLines(between: [Point(x: 144.72135954999578, y: 137.02282018339787),
        ...
        func testUnionCoincidentEdgesRealWorldTestCase2() {
            let star = { () -> Path in
                let temp = CGMutablePath()
                temp.move(to: Point(x: 111.2, y: 90.0))
                temp.addLine(to: Point(x: 144.72135954999578, y: 137.02282018339787))
                temp.addLine(to: Point(x: 89.64133022449836, y: 119.6729633084088))
                temp.addLine(to: Point(x: 55.27864045000421, y: 166.0845213036123))
                temp.addLine(to: Point(x: 54.758669775501644, y: 108.33889987152517))
                temp.addLine(to: Point(x: 0.0, y: 90.00000000000001))
                temp.addLine(to: Point(x: 54.75866977550164, y: 71.66110012847484))
                temp.addLine(to: Point(x: 55.2786404500042, y: 13.915478696387723))
                temp.addLine(to: Point(x: 89.64133022449835, y: 60.3270366915912))
                temp.addLine(to: Point(x: 144.72135954999578, y: 42.97717981660214))
                temp.closeSubpath()
                return Path(cgPath: temp)
            }()
            let polygon = { () -> Path in
                let temp = CGMutablePath()
                temp.move(to: Point(x: 89.64133022449836, y: 119.6729633084088))
                temp.addLine(to: Point(x: 55.27864045000421, y: 166.0845213036123)) // this is marked as an exit if the polygon isn't reversed and it's correct BUT it's unlinked to the other path(!!!)
                temp.addLine(to: Point(x: 143.9588334407257, y: 125.35115333505796))
                temp.addLine(to: Point(x: 160.7501485041311, y: 111.6759272531885))
                temp.closeSubpath()
                return Path(cgPath: temp)
            }()
        ...
        func testUnionRealWorldEdgeCase() {
            guard MemoryLayout<Double>.size > 4 else { return } // not enough precision in points for test to be valid
            let a = { () -> Path in
                let cgPath = CGMutablePath()
                cgPath.move(to: Point(x: 310.198127403852, y: 190.08736919846973))
                cgPath.addCurve(to: Point(x: 309.1982933716744, y: 195.17240727745877),
                                control1: Point(x: 310.390629965343, y: 191.78584973769978),
                                control2: Point(x: 310.0800866088565, y: 193.5583513843498))
                cgPath.addCurve(to: Point(x: 297.52638944557776, y: 198.59685279578636),
                                control1: Point(x: 306.9208206199371, y: 199.34114906559483),
                                control2: Point(x: 301.6951312337138, y: 200.87432554752368))
                cgPath.addCurve(to: Point(x: 293.06807628308206, y: 191.637728075906),
                                control1: Point(x: 294.8541298755864, y: 197.13694026929096),
                                control2: Point(x: 293.26485189217163, y: 194.46557442730858))
                cgPath.addCurve(to: Point(x: 293.0490061981148, y: 191.24674708897507),
                                control1: Point(x: 293.05884562618036, y: 191.50820426365925),
                                control2: Point(x: 293.0524676850055, y: 191.37785711483136))
                cgPath.addCurve(to: Point(x: 301.42017404234923, y: 182.42157189005232),
                                control1: Point(x: 292.9236355289621, y: 186.49810808117778),
                                control2: Point(x: 296.67153503455194, y: 182.546942559205))
                cgPath.addCurve(to: Point(x: 310.198127403852, y: 190.08736919846973),
                                control1: Point(x: 305.9310607601042, y: 182.30247821176928),
                                control2: Point(x: 309.72232986751203, y: 185.6785144367646))
                return Path(cgPath: cgPath)
            }()
            let b = { () -> Path in
                let cgPath = CGMutablePath()
                cgPath.move(to: Point(x: 309.5688043100249, y: 187.66446326122298))
                cgPath.addCurve(to: Point(x: 304.8877314421214, y: 198.89156106846605),
                                control1: Point(x: 311.37643918302956, y: 192.05738329201742),
                                control2: Point(x: 309.28065147291585, y: 197.0839261954614))
                cgPath.addCurve(to: Point(x: 293.6606336348783, y: 194.21048820056248),
                                control1: Point(x: 300.4948114113269, y: 200.6991959414707),
                                control2: Point(x: 295.46826850788295, y: 198.60340823135695))
                cgPath.addCurve(to: Point(x: 298.3417065027818, y: 182.98339039331944),
                                control1: Point(x: 291.85299876187366, y: 189.81756816976807),
                                control2: Point(x: 293.9487864719874, y: 184.79102526632408))
                cgPath.addCurve(to: Point(x: 309.5688043100249, y: 187.66446326122298),
        ...
        func testSubtractingWindingDirection() {
            ...
            let circle = Path(cgPath: CGPath(ellipseIn: Rect(x: 0, y: 0, width: 3, height: 3).cgRect, transform: nil))
            let hole = Path(cgPath: CGPath(ellipseIn: Rect(x: 1, y: 1, width: 1, height: 1).cgRect, transform: nil))
            let donut = circle.subtract(hole)
            XCTAssertTrue(donut.contains(Point(x: 0.5, y: 0.5), using: .winding)) // inside the donut (but not the hole)
            XCTAssertFalse(donut.contains(Point(x: 1.5, y: 1.5), using: .winding)) // center of donut hole
        }
        ...
        func testSubtractingEntirelyErased() {
            ...
            let circle = Path(cgPath: CGPath(ellipseIn: Rect(x: -1, y: -1, width: 2, height: 2).cgRect, transform: nil))
            let biggerCircle = Path(cgPath: CGPath(ellipseIn: Rect(x: -2, y: -2, width: 4, height: 4).cgRect, transform: nil))
        ...
        func testSubtractingEdgeCase1() {
            ...
            let rectangle = Path(cgPath: CGPath(rect: Rect(x: -1, y: -1, width: 4, height: 3).cgRect, transform: nil))
            let circle = Path(cgPath: CGPath(ellipseIn: Rect(x: 0, y: 0, width: 4, height: 4).cgRect, transform: nil))
            ...
            XCTAssertFalse(difference.contains(Point(x: 2.0, y: 2.0)))
        }
        ...
        func testSubtractingEdgeCase2() {
            ...
            let square1 = Path(cgPath: CGPath(rect: Rect(x: 0.0, y: 0.0, width: 2.0, height: 2.0).cgRect, transform: nil))
            let square2CGPath = CGMutablePath()
            square2CGPath.move(to: Point.zero)
            square2CGPath.addLine(to: Point(x: 1.0, y: -1.0))
            square2CGPath.addLine(to: Point(x: 2.0, y: 0.0))
            square2CGPath.addLine(to: Point(x: 1.0, y: 1.0))
            ...
            expectedResultCGPath.move(to: Point.zero)
            expectedResultCGPath.addLine(to: Point(x: 1.0, y: 1.0))
            expectedResultCGPath.addLine(to: Point(x: 2.0, y: 0.0))
            expectedResultCGPath.addLine(to: Point(x: 2.0, y: 2.0))
            expectedResultCGPath.addLine(to: Point(x: 0.0, y: 2.0))
        ...
        func testCrossingsRemoved() {
            ...
            cgPath.closeSubpath()
            let path = Path(cgPath: cgPath)
            let intersection = Point(x: 1.5, y: 1.5)
            ...
            XCTAssertTrue(path.contains(Point(x: 1.5, y: 1.25), using: .winding))
            XCTAssertFalse(path.contains(Point(x: 1.5, y: 1.25), using: .evenOdd))
        ...
        func testCrossingsRemovedIssue64() {
            let outerEllipse: Path = {
                let cgPath = CGPath(ellipseIn: Rect(x: 0, y: 0, width: 5, height: 5).cgRect, transform: nil)
                return Path(cgPath: cgPath)
            }()
            let innerEllipse: Path = {
                let cgPath = CGPath(ellipseIn: Rect(x: 1, y: 1, width: 3, height: 5).cgRect, transform: nil)
            ...
            let point1 = Point(x: 2.5, y: 0.5)
            let point2 = Point(x: 2.5, y: 2.5)
            let point3 = Point(x: 2.5, y: 5.5)
        ...
        func testCrossingsRemovedNoCrossings() {
            ...
            let square = Path(cgPath: CGPath(ellipseIn: Rect(x: 0.0, y: 0.0, width: 1.0, height: 1.0).cgRect, transform: nil))
        ...
        func testCrossingsRemovedEdgeCase() {
            ...
            XCTAssertEqual(contour.windingCount(Point(x: 0.5, y: 0.5)), -1) // winding count at center of one square region
            XCTAssertEqual(contour.windingCount(Point(x: 1.5, y: 1.5)), 1) // winding count at center of other square region

            let crossingsRemoved = contour.crossingsRemoved()

            XCTAssertEqual(crossingsRemoved.components.count, 2)
            let square1: Path = {
                let cgPath = CGPath(rect: Rect(x: 0, y: 0, width: 1, height: 1).cgRect, transform: nil)
                return Path(cgPath: cgPath).reversed()
            }()
            let square2: Path = {
                let cgPath = CGPath(rect: Rect(x: 1, y: 1, width: 1, height: 1).cgRect, transform: nil)
                return Path(cgPath: cgPath)
            }()
        ...
        func testCrossingsRemovedEdgeCaseInnerLoop() {
            ...
            cgPath.move(to: Point.zero)
            cgPath.addLine(to: Point(x: 2.0, y: 0.0))

            // loop in a complete circle back to 2, 0
            cgPath.addArc(tangent1End: Point(x: 3.0, y: 0.0), tangent2End: Point(x: 3.0, y: 1.0), radius: 1)
            cgPath.addArc(tangent1End: Point(x: 3.0, y: 2.0), tangent2End: Point(x: 2.0, y: 2.0), radius: 1)
            cgPath.addArc(tangent1End: Point(x: 1.0, y: 2.0), tangent2End: Point(x: 1.0, y: 1.0), radius: 1)
            cgPath.addArc(tangent1End: Point(x: 1.0, y: 0.0), tangent2End: Point(x: 2.0, y: 0.0), radius: 1)

            // proceed around to close the shape (grazing the loop at (2,2)
            cgPath.addLine(to: Point(x: 4.0, y: 0.0))
            cgPath.addLine(to: Point(x: 4.0, y: 2.0))
            cgPath.addLine(to: Point(x: 2.0, y: 2.0))
            cgPath.addLine(to: Point(x: 0.0, y: 2.0))
            cgPath.closeSubpath()
            ...
            let curves2 = path.components[0].curves.map {
                type(of: $0).init(points: $0.points.map { point in
                    let rounded = Point(x: round(point.x), y: round(point.y))
            ...
            XCTAssertEqual(result.windingCount(Point(x: 0.5, y: 1)), 1)
            XCTAssertEqual(result.windingCount(Point(x: 2.0, y: 1)), 1) // if the inner loop wasn't eliminated we'd have a winding count of 2 here
            XCTAssertEqual(result.windingCount(Point(x: 3.5, y: 1)), 1)
        }
        ...
        func testCrossingsRemovedRealWorldEdgeCaseMagicNumbers() {
            ...
            let cgPath = CGMutablePath()
            let start = Point(x: 79.59559290956605, y: 697.9008011912572)
            cgPath.move(to: start)
            cgPath.addCurve(to: Point(x: 71.31576744881897, y: 729.0705310397749), control1: Point(x: 85.91646553575535, y: 708.7944954952286), control2: Point(x: 82.2094612873204, y: 722.7496586836662))
            cgPath.addCurve(to: Point(x: 40.14603795970622, y: 720.7907053704894), control1: Point(x: 60.4220735042526, y: 735.3914034574259), control2: Point(x: 46.46691031581487, y: 731.6843992089908))
            cgPath.addCurve(to: Point(x: 37.21144227099133, y: 706.7177736592248), control1: Point(x: 39.07549105339858, y: 718.7074812854011), control2: Point(x: 37.21110624960683, y: 711.947464952338))
            cgPath.addCurve(to: Point(x: 62.477966856736, y: 686.6750666235641), control1: Point(x: 38.65395965539626, y: 694.2059748336982), control2: Point(x: 49.96616803120935, y: 685.2325492391592))
            cgPath.addCurve(to: Point(x: 82.52067606376023, y: 711.9415914596509), control1: Point(x: 74.98976785362623, y: 688.1175842583111), control2: Point(x: 83.96319344816517, y: 699.4297926341243))
            cgPath.addCurve(to: start, control1: Point(x: 82.51999960076027, y: 706.7206820370851), control2: Point(x: 80.65889482357387, y: 699.9715389099819))
            let path = Path(cgPath: cgPath)
        ...
        func testCrossingsRemovedAnotherRealWorldCase() {
            ...
            let cgPath = CGMutablePath()
            let start = Point(x: 503.3060153966664, y: 766.9140612367046)
            cgPath.move(to: start)
            cgPath.addCurve(to: Point(x: 517.9306651149989, y: 762.0523534483476),
                            control1: Point(x: 506.0019772976378, y: 761.5330522602719),
                            control2: Point(x: 512.5496560294043, y: 759.3563914926846))
            cgPath.addCurve(to: Point(x: 522.7923732205169, y: 776.6770033255823),
                            control1: Point(x: 523.3116744085926, y: 764.7483155082213),
                            control2: Point(x: 525.4883351761798, y: 771.2959942399877))
            cgPath.addCurve(to: Point(x: 520.758836935199, y: 764.316674774872),
                            control1: Point(x: 522.6619398993569, y: 776.9550303733141), control2: Point(x: 522.7228057838222, y: 776.8532852161298))
            cgPath.addCurve(to: Point(x: 520.6170414159213, y: 779.7723863761416),
                            control1: Point(x: 524.9876580913353, y: 768.6238074338997), control2: Point(x: 524.9241740749491, y: 775.5435652200052))
            cgPath.addCurve(to: Point(x: 505.16132944417086, y: 779.6305912206088),
                            control1: Point(x: 516.3099083864128, y: 784.001207896023),
                            control2: Point(x: 509.3901506003072, y: 783.9377238796366))
            cgPath.addCurve(to: start, control1: Point(x: 503.19076843492786, y: 767.0872665416827), control2: Point(x: 503.3761460381431, y: 766.7563954079359))
            let path = Path(cgPath: cgPath)
        ...
        func testCrossingsRemovedThirdRealWorldCase() {
            let cgPath = CGMutablePath()
            let points = [Point(x: 115.23034681147224, y: 59.327037989273855),
            ...
            cgPath.move(to: Point(x: 130.4334714935808, y: 59.32703798927387))
            cgPath.addLine(to: Point(x: 130.43347149358078, y: 82.92265451611945))
            cgPath.addLine(to: Point(x: 130.4334714935808, y: 215.00646454457666))
            cgPath.addCurve(to: Point(x: 115.23034681147224, y: 215.00646454457666),
                            control1: Point(x: 130.4334714935808, y: 225.1418809993157),
                            control2: Point(x: 115.23034681147224, y: 225.1418809993157))
            cgPath.addLine(to: Point(x: 115.23034681147224, y: 59.32703798927386))
            cgPath.addCurve(to: Point(x: 130.4334714935808, y: 59.32703798927387),
                            control1: Point(x: 115.23034681147224, y: 49.19162153453482),
                            control2: Point(x: 130.4334714935808, y: 49.19162153453483))
        ...
        func testCrosingsRemovedFourthRealWorldCase() {
            ...
            let cgPath = CGMutablePath()
            let firstPoint = Point(x: 128.65039465906003, y: 123.73954643229627)
            cgPath.move(to: firstPoint)
            cgPath.addCurve(to: Point(x: 116.95134864827014, y: 123.73672125818112), control1: Point(x: 125.4190121591063, y: 126.96936863167058), control2: Point(x: 120.18117084764445, y: 126.96810375813484))
            cgPath.addCurve(to: Point(x: 116.95417382238529, y: 112.03767524739123), control1: Point(x: 113.72152644889583, y: 120.5053387582274), control2: Point(x: 113.72279132243156, y: 115.26749744676555))
            cgPath.addCurve(to: Point(x: 117.06818455296886, y: 111.94933998303057), control1: Point(x: 119.3560792543184, y: 110.34087389676174), control2: Point(x: 120.25529993069892, y: 109.98254275757822))
            cgPath.addCurve(to: Point(x: 128.80664909167646, y: 111.95922916966808), control1: Point(x: 120.31240285203181, y: 108.71058333093575), control2: Point(x: 125.56789243958164, y: 108.71501087060513))
            cgPath.addCurve(to: Point(x: 128.79675990503895, y: 123.69769370837568), control1: Point(x: 132.04540574377128, y: 115.20344746873103), control2: Point(x: 132.0409782041019, y: 120.45893705628086))
            cgPath.addCurve(to: firstPoint, control1: Point(x: 125.59151708590264, y: 125.68258785765616), control2: Point(x: 126.31169113142379, y: 125.37317639620701))
            let path = Path(cgPath: cgPath)
            let result = path.crossingsRemoved(accuracy: 1.0e-4)
            let point1 = Point(x: 128.50258215906004, y: 123.86146049479626)
            let point2 = Point(x: 128.64870715906002, y: 123.77228080729627)
            let point3 = Point(x: 127.29466809656003, y: 124.65276518229626)
        ...
        func testCrossingsRemovedRealWorldInfiniteLoop() {
            ...
            let cgPath = CGMutablePath()
            cgPath.move(to: Point(x: 431.2394694928875, y: 109.81690300533613))
            cgPath.addCurve(to: Point(x: 430.66935231730844, y: 110.3870201809152),
                            control1: Point(x: 431.2394694928875, y: 110.13177002702506),
                            control2: Point(x: 430.9842193389974, y: 110.3870201809152))
            cgPath.addLine(to: Point(x: 382.89122776801867, y: 110.3870201809152))
            cgPath.addLine(to: Point(x: 383.46134494359774, y: 109.81690300533613))
            cgPath.addLine(to: Point(x: 383.46134494359774, y: 125.44498541142156))
            cgPath.addLine(to: Point(x: 382.89122776801867, y: 124.87486823584248))
            cgPath.addLine(to: Point(x: 430.66935231730844, y: 124.87486823584248))
            cgPath.addLine(to: Point(x: 430.09923514172937, y: 125.44498541142156))
            cgPath.addLine(to: Point(x: 430.09923514172937, y: 99.92396144754883))
            cgPath.addLine(to: Point(x: 431.2394694928875, y: 99.92396144754883))
            cgPath.closeSubpath()

            cgPath.move(to: Point(x: 430.09923514172937, y: 109.81690300533613))
            cgPath.addLine(to: Point(x: 430.09923514172937, y: 99.92396144754883))
            cgPath.addCurve(to: Point(x: 431.2394694928875, y: 99.92396144754883),
                            control1: Point(x: 430.09923514172937, y: 99.16380521344341),
                            control2: Point(x: 431.2394694928875, y: 99.16380521344341))
            cgPath.addLine(to: Point(x: 431.2394694928875, y: 125.44498541142156))
            cgPath.addCurve(to: Point(x: 430.66935231730844, y: 126.01510258700063),
                            control1: Point(x: 431.2394694928875, y: 125.75985243311048),
                            control2: Point(x: 430.9842193389974, y: 126.01510258700063))
            cgPath.addLine(to: Point(x: 382.89122776801867, y: 126.01510258700063))
            cgPath.addCurve(to: Point(x: 382.3211105924396, y: 125.44498541142156),
                            control1: Point(x: 382.5763607463297, y: 126.01510258700063),
                            control2: Point(x: 382.3211105924396, y: 125.75985243311048))
            cgPath.addLine(to: Point(x: 382.3211105924396, y: 109.81690300533613))
            cgPath.addCurve(to: Point(x: 382.89122776801867, y: 109.24678582975706),
                            control1: Point(x: 382.3211105924396, y: 109.5020359836472),
                            control2: Point(x: 382.5763607463297, y: 109.24678582975706))
            cgPath.addLine(to: Point(x: 430.66935231730844, y: 109.24678582975706))
            cgPath.closeSubpath()
...
//
//        let openPath = Path(curve: CubicCurve(p0: Point(x: 1, y: 1),
//                                              p1: Point(x: 2, y: 2),
//                                              p2: Point(x: 4, y: 0),
//                                              p3: Point(x: 5, y: 1)))
//        let closedPath = Path(cgPath: CGPath(rect: Rect(x: 0, y: 0, width: 2, height: 2).cgRect, transform: nil))
//
...
//    }
        ...
        func testOffset() {
            let circle = Path(cgPath: CGPath(ellipseIn: Rect(x: 0, y: 0, width: 2, height: 2).cgRect, transform: nil)) // ellipse with radius 1 centered at 1,1
            ...
            let expectedRadius = 2.0
            let expectedCenter = Point(x: 1.0, y: 1.0)
        ...
        func testOffsetDegenerate() {
            ...
            let point = Point(x: 245.2276926738644, y: 76.62374839782714)
        ...
        func testDisjointComponentsNesting() {
            ...
            let squarePath = Path(cgPath: CGPath(rect: Rect(x: 0, y: 0, width: 7, height: 7).cgRect, transform: nil))
            ...
            let squareWithHolePath = { () -> Path in
                let cgPath = CGPath(rect: Rect(x: 1, y: 1, width: 5, height: 5).cgRect, transform: nil)
            ...
            let pegPath = Path(cgPath: CGPath(rect: Rect(x: 2, y: 2, width: 3, height: 3).cgRect, transform: nil))
            ...
            let pegWithHolePath = { () -> Path in
                let cgPath = CGPath(rect: Rect(x: 3, y: 3, width: 1, height: 1).cgRect, transform: nil)
                let hole = Path(cgPath: cgPath).reversed()
        ...
        func testDisjointComponentsWindingBackwards() {
            let innerSquare = Path(cgPath: CGPath(rect: Rect(x: 2, y: 2, width: 1, height: 1).cgRect, transform: nil))
            let hole = Path(cgPath: CGPath(rect: Rect(x: 1, y: 1, width: 3, height: 3).cgRect, transform: nil)).reversed()
            let outerSquare = Path(cgPath: CGPath(rect: Rect(x: 0, y: 0, width: 5, height: 5).cgRect, transform: nil))
            ...
            for expectedPath in expectedPaths {
                XCTAssert(disjointPaths.contains(expectedPath))
            }
        }
        ...
        func testApply() {
            ...
            let pointPathComponent = PathComponent(points: [Point(x: 3, y: 5)], orders: [0])
            clearRecordsAndGetPointer { recordsPointer in
                pointPathComponent.apply(info: recordsPointer, function: applierFunction)
            }
            XCTAssertEqual(records, [CGPathElementRecord(type: .moveToPoint, points: [Point(x: 3, y: 5)])])
    ...
    func testBoundingBoxOfPath() {
        ...
        let quad1 = QuadraticCurve(p0: Point(x: 1, y: 2),
                                   p1: Point(x: 2, y: 4),
                                   p2: Point(x: 3, y: 2))
        let quad2 = QuadraticCurve(p0: Point(x: 3, y: 2),
                                   p1: Point(x: 2, y: 0),
                                   p2: Point(x: 1, y: 2))
        let path1 = Path(curve: quad1)
        XCTAssertEqual(path1.boundingBoxOfPath, BoundingBox(p1: Point(x: 1, y: 2), p2: Point(x: 3, y: 4)))
        let path2 = Path(components: [PathComponent(curve: quad1),
                                      PathComponent(curve: quad2)])
        XCTAssertEqual(path2.boundingBoxOfPath, BoundingBox(p1: Point(x: 1, y: 0), p2: Point(x: 3, y: 4)))
    }

    #if !os(WASI)
        func testNSCoder() {
            // just some random curves, but we ensure they're continuous
            let l1 = LineSegment(p0: Point(x: 4.9652, y: 8.2774),
                                 p1: Point(x: 3.8449, y: 4.9902))
            let q1 = QuadraticCurve(p0: Point(x: 3.8449, y: 4.9902),
                                    p1: Point(x: 4.0766, y: 7.0715),
                                    p2: Point(x: 7.7088, y: 8.6246))
            let l2 = LineSegment(p0: Point(x: 7.7088, y: 8.6246),
                                 p1: Point(x: 3.6054, y: 3.0114))
            let c1 = CubicCurve(p0: Point(x: 3.6054, y: 3.0114),
