...
class CubicCurveTests: XCTestCase {
    ...
    func testInitializerArray() {
        let c = CubicCurve(points: [Point(x: 1.0, y: 1.0), Point(x: 3.0, y: 2.0), Point(x: 5.0, y: 3.0), Point(x: 7.0, y: 4.0)])
        ...
        XCTAssertEqual(c.startingPoint, Point(x: 1.0, y: 1.0))
        XCTAssertEqual(c.endingPoint, Point(x: 7.0, y: 4.0))
    }

    func testInitializerIndividualPoints() {
        let c = CubicCurve(p0: Point(x: 1.0, y: 1.0), p1: Point(x: 3.0, y: 2.0), p2: Point(x: 5.0, y: 3.0), p3: Point(x: 7.0, y: 4.0))
        XCTAssertEqual(c.p0, Point(x: 1.0, y: 1.0))
        XCTAssertEqual(c.p1, Point(x: 3.0, y: 2.0))
        XCTAssertEqual(c.p2, Point(x: 5.0, y: 3.0))
        XCTAssertEqual(c.p3, Point(x: 7.0, y: 4.0))
    ...
    func testInitializerLineSegment() {
        let l = LineSegment(p0: Point(x: 1.0, y: 2.0), p1: Point(x: 2.0, y: 3.0))
    ...
    func testInitializerQuadratic() {
        let q = QuadraticCurve(p0: Point(x: 1.0, y: 1.0), p1: Point(x: 2.0, y: 2.0), p2: Point(x: 3.0, y: 1.0))
        ...
        XCTAssert((c.p0 - Point(x: 1.0, y: 1.0)).length < epsilon)
        XCTAssert((c.p1 - Point(x: fiveThirds, y: fiveThirds)).length < epsilon)
        XCTAssert((c.p2 - Point(x: sevenThirds, y: fiveThirds)).length < epsilon)
        XCTAssert((c.p3 - Point(x: 3.0, y: 1.0)).length < epsilon)
    }
    ...
    func testBasicProperties() {
        let c = CubicCurve(p0: Point(x: 1.0, y: 1.0), p1: Point(x: 3.0, y: 2.0), p2: Point(x: 4.0, y: 2.0), p3: Point(x: 6.0, y: 1.0))
        XCTAssert(c.simple)
        XCTAssertEqual(c.order, 3)
        XCTAssertEqual(c.startingPoint, Point(x: 1.0, y: 1.0))
        XCTAssertEqual(c.endingPoint, Point(x: 6.0, y: 1.0))
    }

    func testSetStartEndPoints() {
        var c = CubicCurve(p0: Point(x: 5.0, y: 6.0), p1: Point(x: 6.0, y: 5.0), p2: Point(x: 7.0, y: 8.0), p3: Point(x: 8.0, y: 7.0))
        c.startingPoint = Point(x: 4.0, y: 5.0)
        XCTAssertEqual(c.p0, c.startingPoint)
        XCTAssertEqual(c.startingPoint, Point(x: 4.0, y: 5.0))
        c.endingPoint = Point(x: 9.0, y: 8.0)
        XCTAssertEqual(c.p3, c.endingPoint)
        XCTAssertEqual(c.endingPoint, Point(x: 9.0, y: 8.0))
    }

    func testSimple() {
        // create a simple cubic curve (very simple, because it's equal to a line segment)
        let c1 = CubicCurve(p0: Point(x: 1.0, y: 1.0), p1: Point(x: 2.0, y: 2.0), p2: Point(x: 3.0, y: 3.0), p3: Point(x: 4.0, y: 4.0))
        XCTAssertTrue(c1.simple)
        // a non-trivial example of a simple curve -- almost a straight line
        let c2 = CubicCurve(p0: Point(x: 1.0, y: 1.0), p1: Point(x: 2.0, y: 1.05), p2: Point(x: 3.0, y: 1.05), p3: Point(x: 4.0, y: 1.0))
        XCTAssertTrue(c2.simple)
        // non-simple curve, control points fall on different sides of the baseline
        let c3 = CubicCurve(p0: Point(x: 1.0, y: 1.0), p1: Point(x: 2.0, y: 1.05), p2: Point(x: 3.0, y: 0.95), p3: Point(x: 4.0, y: 1.0))
        ...
        let c4 = CubicCurve(p0: Point(x: 1.0, y: 1.0), p1: Point(x: 1.0, y: 2.0), p2: Point(x: 2.0, y: 3.0), p3: Point(x: 3.0, y: 3.0))
        XCTAssertFalse(c4.simple)
        // ensure that points-as-cubics pass (otherwise callers might try to subdivide them further)
        let p = Point(x: 1.234, y: 5.689)
    ...
    func testDerivative() {
        ...
        XCTAssert(distance(c.derivative(at: 0.5), Point(x: 6.0, y: 0.0)) < epsilon)
    ...
    func testSplitFromToSameLocation() {
        ...
        let cubic = CubicCurve(p0: Point(x: 0.041630344771878214, y: 0.45449244472862915),
    ...
    func testSplitContinuous() {
        ...
        let a = 0.65472931005125345
        let b = 0.73653845530600293
        let c = 1.0
        let curve = CubicCurve(p0: Point(x: 286.8966218087201, y: 69.11759651620365),
    ...
    func testBoundingBox() {
        ...
        let c1 = CubicCurve(p0: Point(x: 1.0, y: 1.0), p1: Point(x: 3.0, y: 2.0), p2: Point(x: 5.0, y: 2.0), p3: Point(x: 7.0, y: 1.0))
        ...
        let c2 = CubicCurve(p0: Point(x: 1.0, y: 1.0), p1: Point(x: -3.0, y: 2.0), p2: Point(x: -3.0, y: 3.0), p3: Point(x: 1.0, y: 4.0))
    ...
    func testLength() {
        let epsilon = 0.00001
        let c1 = CubicCurve(p0: Point(x: 1.0, y: 2.0),
                            p1: Point(x: 7.0 / 3.0, y: 3.0),
                            p2: Point(x: 11.0 / 3.0, y: 4.0),
                            p3: Point(x: 5.0, y: 5.0)) // represents a straight line of length 5 -- most curves won't have an easy reference solution
    ...
    func testProject() {
        let epsilon = 1.0e-5
        // test a cubic
        let c = CubicCurve(p0: Point(x: 1.0, y: 1.0), p1: Point(x: 2.0, y: 2.0), p2: Point(x: 4.0, y: 2.0), p3: Point(x: 5.0, y: 1.0))
        let p4 = c.project(Point(x: 0.95, y: 1.05)) // should project to p0
        XCTAssertEqual(p4.point, Point(x: 1.0, y: 1.0))
        XCTAssertEqual(p4.t, 0.0)
        let p5 = c.project(Point(x: 5.05, y: 1.05)) // should project to p3
        XCTAssertEqual(p5.point, Point(x: 5.0, y: 1.0))
        XCTAssertEqual(p5.t, 1.0)
        let p6 = c.project(Point(x: 3.0, y: 2.0)) // should project to center of curve
        XCTAssertEqual(p6.point, Point(x: 3.0, y: 1.75))
    ...
    func testProjectRealWorldIssue() {
        ...
        let epsilon = 1.0e-5
        let c = CubicCurve(p0: Point(x: 100, y: 25),
                           p1: Point(x: 10, y: 90),
                           p2: Point(x: 50, y: 185),
                           p3: Point(x: 170, y: 175))
        let t = c.project(Point(x: 8.3359375, y: -49.10546875)).t
...
//    }
    ...
    func testNormalDegenerate() {
        ...
        XCTAssertTrue(distance(cubic1.normal(at: 0), Point(x: 0, y: 1)) < maxError)
        let cubic2 = CubicCurve(p0: a, p1: b, p2: c, p3: c)
        XCTAssertTrue(distance(cubic2.normal(at: 1), Point(x: -sqrt(2) / 2, y: sqrt(2) / 2)) < maxError)
        let cubic3 = CubicCurve(p0: a, p1: a, p2: a, p3: b)
        XCTAssertTrue(distance(cubic3.normal(at: 0), Point(x: 0, y: 1)) < maxError)
        let cubic4 = CubicCurve(p0: a, p1: b, p2: b, p3: b)
        XCTAssertTrue(distance(cubic4.normal(at: 1), Point(x: 0, y: 1)) < maxError)
    }
    ...
    func testNormalCusp() {
        ...
        let c = CubicCurve(p0: Point(x: 1, y: 1),
                           ...
                           p2: Point(x: 1, y: 2),
                           p3: Point(x: 2, y: 1))
        XCTAssertEqual(c.derivative(at: 0.5), Point.zero)
    ...
    func testReduce() {
        ...
        let c1 = CubicCurve(p0: Point(x: 0.0, y: 0.0),
        ...
        let c2 = CubicCurve(p0: Point(x: 0.0, y: 0.0),
    ...
    func testReduceExtremaCloseby() {
        ...
        let smallValue = 1.0e-3
        let c = BezierKitTestHelpers.cubicCurveFromPolynomials([0, 1, -1, 0.25], [Double(1.0 / 3.0), Double(-1.0 / 2.0) + smallValue, Double(3.0 / 16.0), 0])
...
//        let q: QuadraticCurve = QuadraticCurve.init(p0: Point(x: 0.0, y: 0.0),
//                                                                p1: Point(x: 1.0, y: 2.0),
...
//        let r1: Double = 1.0 - sqrt(1.0 / 2.0)
//        let r2: Double = 1.0 + sqrt(1.0 / 2.0)
...
//        // we mostly just care that we call into the proper implementation and that the results are ordered correctly
//        let epsilon: Double = 0.00001
//        let c: CubicCurve = CubicCurve(p0: Point(x: -1, y: 0),
//                                                   p1: Point(x: -1, y: 1),
//                                                   p2: Point(x:  1, y: -1),
//                                                   p3: Point(x:  1, y: 0))
//        let l1: LineSegment = LineSegment(p0: Point(x: -2.0, y: 0.0), p1: Point(x: 2.0, y: 0.0))
...
//        let l2: LineSegment = LineSegment(p0: Point(x: 2.0, y: 0.0), p1: Point(x: -2.0, y: 0.0))
...
//
    ...
    func testIntersectionsCubicMaxIntersections() {
        let epsilon = 1.0e-5
        let a = 4.0
        let c1 = CubicCurve(p0: Point(x: 0, y: 0),
                            p1: Point(x: 0.33, y: a),
                            p2: Point(x: 0.66, y: 1 - a),
                            p3: Point(x: 1, y: 1))
        let c2 = CubicCurve(p0: Point(x: 0, y: 1),
                            p1: Point(x: a, y: 0.66),
                            p2: Point(x: 1 - a, y: 0.33),
                            p3: Point(x: 1, y: 0))
        let intersections = c1.intersections(with: c2, accuracy: epsilon)
        let expectedResults = [Point(x: 0.009867618966216286, y: 0.11635072599233257),
    ...
    func testBasicTangentIntersection() {
        let c1 = CubicCurve(p0: Point(x: 0, y: 0),
                            ...
                            p2: Point(x: 6, y: 9),
                            p3: Point(x: 9, y: 9))
        let c2 = CubicCurve(p0: Point(x: 9, y: 9),
                            p1: Point(x: 8, y: 9),
                            p2: Point(x: 6, y: 7),
                            p3: Point(x: 6, y: 6))
    ...
    // Skip on platforms where Double is 32bit
        ...
        func testRealWorldNearlyCoincidentCurvesIntersection() {
            ...
            let c1 = CubicCurve(p0: Point(x: 0.9435597332840757, y: 0.16732142729460975),
            ...
            let c2 = CubicCurve(p0: Point(x: 0.036560070230819974, y: 0.28765861655756453),
    ...
    func testIntersectionsCubicButActuallyLinear() {
        ...
        let epsilon = 1.0e-5
        let cubicButActuallyLinear = CubicCurve(p0: Point(x: 3, y: 2),
                                                p1: Point(x: 4, y: 3),
                                                p2: Point(x: 5, y: 4),
                                                p3: Point(x: 6, y: 5))
        let cubic = CubicCurve(p0: Point(x: 1, y: 0),
                               p1: Point(x: 3, y: 6),
                               p2: Point(x: 5, y: 2),
                               p3: Point(x: 7, y: 0))
    ...
    func testIntersectionsCubicButActuallyQuadratic() {
        ...
        // the implicit equation will be f(x, y) = 0 and no intersections will be found
        let epsilon = 1.0e-5
        let cubicButActuallyQuadratic = CubicCurve(p0: Point(x: 1, y: 1),
                                                   p1: Point(x: 2, y: 4),
                                                   p2: Point(x: 3, y: 4),
                                                   p3: Point(x: 4, y: 1))
        let cubic = CubicCurve(p0: Point(x: 0, y: 0),
                               p1: Point(x: 2, y: 4),
                               p2: Point(x: 4, y: 3),
                               p3: Point(x: 6, y: 3))
    ...
    func testRealWorldPrecisionIssue() {
        ...
        let c1 = CubicCurve(p0: Point(x: 94.9790542640437, y: 96.49280906706511),
        ...
        let c2 = CubicCurve(p0: Point(x: 123.54200084128175, y: 48.71908399606449),
    ...
    func testRealWorldInversionIssue() {
        ...
        let c1 = CubicCurve(p0: Point(x: 314.9306297035616, y: 2211.1494686514056),
        ...
        let c2 = CubicCurve(p0: Point(x: 314.8254662024578, y: 2210.9959498495646),
    ...
    func testCubicIntersectsLine() {
        ...
        let c = CubicCurve(p0: Point(x: -1, y: 0),
                           ...
                           p2: Point(x: 1, y: -1),
                           p3: Point(x: 1, y: 0))
        let l: BezierCurve = LineSegment(p0: Point(x: -2.0, y: 0.0), p1: Point(x: 2.0, y: 0.0))
    ...
    func testCubicIntersectsLineEdgeCase() {
        ...
        let c = CubicCurve(p0: Point(x: 3, y: 1),
                           p1: Point(x: 3, y: 1.5522847498307932),
                           p2: Point(x: 2.5522847498307932, y: 2),
                           p3: Point(x: 2, y: 2))
        let l = LineSegment(p0: Point(x: 2, y: 2), p1: Point(x: 0, y: 2))
        let i = c.intersections(with: l)
    ...
    func testCubicIntersectsLineCoincident() {
        let line = LineSegment(p0: Point(x: -4, y: 7), p1: Point(x: 10, y: 3))
    ...
    func testEquatable() {
        ...
        let c2 = CubicCurve(p0: Point(x: 5.0, y: 6.0), p1: p1, p2: p2, p3: p3)
        let c3 = CubicCurve(p0: p0, p1: Point(x: 1.0, y: 3.0), p2: p2, p3: p3)
        let c4 = CubicCurve(p0: p0, p1: p1, p2: Point(x: 3.0, y: 6.0), p3: p3)
        let c5 = CubicCurve(p0: p0, p1: p1, p2: p2, p3: Point(x: -4.0, y: 2.0))
