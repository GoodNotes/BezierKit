...
class BezierCurve_ImplicitizationTests: XCTestCase {
    func testLineSegmentImplicitization() {
        let lineSegment = LineSegment(p0: Point(x: 1, y: 2), p1: Point(x: 4, y: 3))
        ...
        XCTAssertEqual(implicitLine.value(at: Point(x: 0, y: 5)), 10)
        XCTAssertEqual(implicitLine.value(at: Point(x: 2, y: -1)), -10)
        // check the implicit line composed with a parametric line
        let otherLineSegment = LineSegment(p0: Point(x: 0, y: 5), p1: Point(x: 2, y: -1))
    ...
    func testQuadraticCurveImplicitization() {
        let quadraticCurve = QuadraticCurve(p0: Point(x: 0, y: 2),
                                            p1: Point(x: 1, y: 0),
                                            p2: Point(x: 2, y: 2))
        ...
        let valueAbove = implicitQuadratic.value(at: Point(x: 1, y: 2))
        let valueBelow = implicitQuadratic.value(at: Point(x: 1, y: 0))
        ...
        let otherQuadratic = QuadraticCurve(p0: Point(x: 1, y: 2),
                                            p1: Point(x: 1, y: 1),
                                            p2: Point(x: 1, y: 0))
    ...
    func testCubicImplicitization() {
        let cubicCurve = CubicCurve(p0: Point(x: 0, y: 0),
                                    p1: Point(x: 1, y: 1),
                                    p2: Point(x: 2, y: 0),
                                    p3: Point(x: 3, y: 1))
        ...
        let valueAbove = implicitCubic.value(at: Point(x: 1, y: 1))
        let valueBelow = implicitCubic.value(at: Point(x: 2, y: 0))
        XCTAssertGreaterThan(valueAbove, 0)
        XCTAssertLessThan(valueBelow, 0)
        // check the implicit quadratic composed with a parametric line
        let lineSegment = LineSegment(p0: Point(x: 1, y: 1), p1: Point(x: 2, y: 0))
