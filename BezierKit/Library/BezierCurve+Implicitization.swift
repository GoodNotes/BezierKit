...
struct ImplicitPolynomial {
    ...
    fileprivate init(coefficients: [Double], order: Int) {
    ...
    private func coefficient(_ i: Int, _ j: Int) -> Double {
    ...
    func value<P: BernsteinPolynomial>(_ x: P, _ y: P) -> BernsteinPolynomialN {
        ...
        var sum = BernsteinPolynomialN(coefficients: [Double](repeating: 0, count: resultOrder + 1))
        for i in 0 ... order {
            let xPower: BernsteinPolynomialN = xPowers[i]
            for j in 0 ... order {
                let c: Double = coefficient(i, j)
    ...
    func value(at point: Point) -> Double {
        let x = point.x
        let y = point.y
        var sum: Double = 0
        for i in 0 ... order {
...
private struct ImplicitLine {
    ...
    static func * (left: Double, right: ImplicitLine) -> ImplicitLine {
...
private extension BezierCurve {
    func l(_ i: Int, _ j: Int) -> ImplicitLine {
        ...
        let pi = points[i]
        let pj = points[j]
        let b = Double(Utils.binomialCoefficient(n, choose: i) * Utils.binomialCoefficient(n, choose: j))
        return b * ImplicitLine(a10: pi.y - pj.y, a01: pj.x - pi.x, a00: pi.x * pj.y - pj.x * pi.y)
    }
}

extension LineSegment: Implicitizeable {
    var implicitPolynomial: ImplicitPolynomial {
