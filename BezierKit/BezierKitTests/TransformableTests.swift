...
import XCTest
    ...
    class TransformableTests: XCTestCase {
        ...
        let transform = AffineTransform(a: 0, b: 1, c: -1, d: 0, tx: -1, ty: 1)
        ...
        func testTransform() {
            ...
            XCTAssertEqual(Point(x: 1, y: 0).applying(transform), Point(x: -1, y: 2))
        }

        func testTransformLineSegment() {
            let l = LineSegment(p0: Point(x: -1, y: -1), p1: Point(x: 3, y: 1))
            XCTAssertEqual(l.copy(using: transform), LineSegment(p0: Point(x: 0, y: 0), p1: Point(x: -2, y: 4)))
        }
        ...
        func testTransformQuadraticCurve() {
            let q = QuadraticCurve(p0: Point(x: -1, y: -1),
                                   p1: Point(x: 3, y: 1),
                                   p2: Point(x: 7, y: -1))
            XCTAssertEqual(q.copy(using: transform), QuadraticCurve(p0: Point(x: 0, y: 0),
                                                                    p1: Point(x: -2, y: 4),
                                                                    p2: Point(x: 0, y: 8)))
        }
        ...
        func testTransformCubicCurve() {
            let c = CubicCurve(p0: Point(x: -1, y: -1),
                               ...
                               p2: Point(x: 7, y: -1),
                               p3: Point(x: 8, y: 0))
            XCTAssertEqual(c.copy(using: transform), CubicCurve(p0: Point(x: 0, y: 0),
                                                                p1: Point(x: -2, y: 4),
                                                                p2: Point(x: 0, y: 8),
                                                                p3: Point(x: -1, y: 9)))
        }
        ...
        func testTransformPathComponent() {
            let line = LineSegment(p0: Point(x: -1, y: -1), p1: Point(x: 3, y: 1))
            ...
            XCTAssertEqual(transformedComponent.curves.first as? LineSegment, LineSegment(p0: Point(x: 0, y: 0), p1: Point(x: -2, y: 4)))
        }

        func testTransformPath() {
            // just a simple path with two path components made up of line segments

            let l1 = LineSegment(p0: Point(x: -1, y: -1), p1: Point(x: 3, y: 1))
            let l2 = LineSegment(p0: Point(x: 1, y: 1), p1: Point(x: 2, y: 3))

            let path = Path(components: [PathComponent(curves: [l1]), PathComponent(curves: [l2])])

            let transformedPath = path.copy(using: transform)

            let expectedl1 = LineSegment(p0: Point(x: 0, y: 0), p1: Point(x: -2, y: 4))
            let expectedl2 = LineSegment(p0: Point(x: -2, y: 2), p1: Point(x: -4, y: 3))
