...
struct BernsteinPolynomialN: BernsteinPolynomial {
    ...
    func difference(a1 _: Double, a2 _: Double) -> BernsteinPolynomialN {
    ...
    var derivative: BernsteinPolynomialN {
        guard order > 0 else { return BernsteinPolynomialN(coefficients: [Double.zero]) }
        return Double(order) * hodograph
    }

    func value(at t: Double) -> Double {
        return split(at: t).left.coefficients.last!
    }
...
extension BernsteinPolynomialN {
    ...
    static func * (left: BernsteinPolynomialN, right: BernsteinPolynomialN) -> BernsteinPolynomialN {
        ...
        let points = (0 ... m + n).map { k -> Double in
            let start = max(k - n, 0)
            let end = min(m, k)
            let sum = (start ... end).reduce(Double.zero) { totalSoFar, i in
                let j = k - i
                return totalSoFar + Double(Utils.binomialCoefficient(m, choose: i) * Utils.binomialCoefficient(n, choose: j)) * left.coefficients[i] * right.coefficients[j]
            }
