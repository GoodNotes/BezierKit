...
import XCTest
    ...
    class PathComponentProjectionTests: XCTestCase {
        func testProject() {
            let line = PathComponent(curve: LineSegment(p0: Point(x: 1, y: 1), p1: Point(x: 2, y: 2)))
            let point1 = Point(x: 1, y: 2)
            let result1 = line.project(point1)
            XCTAssertEqual(result1.point, Point(x: 1.5, y: 1.5))
