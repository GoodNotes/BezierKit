...
class CGPointTests: XCTestCase {
    func testOperators() {
        let p1 = Point(x: 1.25, y: 2.0)
        let p2 = Point(x: -3.0, y: 4.5)
        XCTAssertEqual(p1 + p2, Point(x: -1.75, y: 6.5))
        XCTAssertEqual(p1 - p2, Point(x: 4.25, y: -2.5))
        XCTAssertEqual(2.0 * p1, Point(x: 2.5, y: 4.0))
        XCTAssertEqual(p2 / 0.5, Point(x: -6.0, y: 9.0))
        XCTAssertEqual(-p2, Point(x: 3.0, y: -4.5))
        XCTAssertEqual(p1[0], 1.25)
        XCTAssertEqual(p1[1], 2.0)

        var p3 = Point(x: 5.0, y: 3.0)
        p3 += Point(x: 1.0, y: -2.0)
        XCTAssertEqual(p3, Point(x: 6.0, y: 1.0))

        var p4 = Point(x: 2.0, y: 9.0)
        p4 -= Point(x: 2.0, y: 8.0)
        XCTAssertEqual(p4, Point(x: 0.0, y: 1.0))
    ...
    func testFunctions() {
        ...
        XCTAssertEqual(a.normalize(), Point(x: 3.0 / 5.0, y: 4.0 / 5.0))
        XCTAssertEqual(distance(a, b), sqrt(17.0))
        XCTAssertEqual(distanceSquared(a, b), 17.0)
        XCTAssertEqual(a.perpendicular, Point(x: -4, y: 3))
    }
}
