...
class BezierCurve_PolynomialTests: XCTestCase {
    func testPolynomialLineSegment() {
        let lineSegment = LineSegment(p0: Point(x: 3, y: 4), p1: Point(x: 5, y: 6))
    ...
    func testPolynomialQuadratic() {
        let quadratic = QuadraticCurve(p0: Point(x: 1, y: 0),
                                       p1: Point(x: 2, y: -2),
                                       p2: Point(x: 3, y: -1))
    ...
    func testExtremaLine() {
        let l1 = LineSegment(p0: Point(x: 1.0, y: 2.0), p1: Point(x: 4.0, y: 6.0))
        let (x1, y1, all1) = l1.extrema()
        XCTAssertTrue(x1.isEmpty)
        XCTAssertTrue(y1.isEmpty)
        XCTAssertTrue(all1.isEmpty)

        let l2 = LineSegment(p0: Point(x: 1.0, y: 2.0), p1: Point(x: 4.0, y: 2.0))
    ...
    func testExtremaQuadratic() {
        let f: [Double] = [4, -2, 1] // f(t) = 4t^2 - 2t + 1, which has a local minimum at t = 0.25
        let g: [Double] = [1, -4, 4] // g(t) = t^2 -4t + 4, which has a local minimum at t = 2 (outside parameter range)
    ...
    func testExtremaCubic() {
        ...
        let g: [Double] = [0, 3, -2, 0] // g(t) = 3t^2 - 2t, which has a local minimum at t=1/3
